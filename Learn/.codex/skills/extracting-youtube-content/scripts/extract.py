...
DEFAULT_SLEEP = 20.0
DEFAULT_SLEEP_JITTER = 5.0
DEFAULT_IP_BLOCK_COOLDOWN = 180.0
DEFAULT_IP_BLOCK_COOLDOWN_JITTER = 60.0
DEFAULT_MAX_CONSECUTIVE_IP_BLOCKS = 3
IP_BLOCK_ERROR_TYPES = {"IpBlocked", "RequestBlocked"}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--sleep-jitter", type=float, default=DEFAULT_SLEEP_JITTER,
                   help=f"Random extra seconds added after each video (default: {DEFAULT_SLEEP_JITTER}).")
    p.add_argument("--ip-block-cooldown", type=float, default=DEFAULT_IP_BLOCK_COOLDOWN,
                   help=f"Seconds to pause after IpBlocked / RequestBlocked (default: {DEFAULT_IP_BLOCK_COOLDOWN}).")
    p.add_argument("--ip-block-cooldown-jitter", type=float, default=DEFAULT_IP_BLOCK_COOLDOWN_JITTER,
                   help=f"Random extra cooldown seconds after IpBlocked / RequestBlocked (default: {DEFAULT_IP_BLOCK_COOLDOWN_JITTER}).")
    p.add_argument("--max-consecutive-ip-blocks", type=int, default=DEFAULT_MAX_CONSECUTIVE_IP_BLOCKS,
                   help=f"Stop batch after this many consecutive IP blocks; 0 disables (default: {DEFAULT_MAX_CONSECUTIVE_IP_BLOCKS}).")
...
def is_ip_block_error(error_type: str | None) -> bool:
    """True for transcript-api errors that mean YouTube is blocking this network path."""
    return error_type in IP_BLOCK_ERROR_TYPES


def is_ip_block_result(result: dict) -> bool:
    """Return True when a per-video result should trigger batch-level cooldown."""
    return (
        is_ip_block_error(result.get("transcript_error_type"))
        or is_ip_block_error(result.get("error_type"))
        or any(name in str(result.get("error", "")) for name in IP_BLOCK_ERROR_TYPES)
    )


def jittered_delay(base_seconds: float, jitter_seconds: float) -> float:
    """Return a non-negative delay with one-sided jitter to avoid fixed request cadence."""
...
def process_one(
    ...
    return {
        ...
        "transcript_error_type":   transcript_error_type,
        "transcript_error_stage":  transcript_error_stage,
        "ip_blocked":              is_ip_block_error(transcript_error_type),
        "original_language":       original_language,
        "chapters_usable":         is_chapters_usable,
...
def main(argv: list[str] | None = None) -> int:
    ...
    for i, vid in enumerate(video_ids, 1):
        ...
        if is_ip_block_result(res):
            consecutive_ip_blocks += 1
            if args.max_consecutive_ip_blocks > 0 and consecutive_ip_blocks >= args.max_consecutive_ip_blocks:
                print(
                    f"[warn] stopping after {consecutive_ip_blocks} consecutive IP blocks "
                    f"(--max-consecutive-ip-blocks={args.max_consecutive_ip_blocks})",
                    file=sys.stderr,
                )
                break
            if i < len(video_ids):
                delay = jittered_delay(args.ip_block_cooldown, args.ip_block_cooldown_jitter)
                print(f"[warn] IP block detected; cooling down for {delay:.1f}s", file=sys.stderr)
                time.sleep(delay)
            continue
        ...
        if not res.get("skipped"):
            consecutive_ip_blocks = 0
        if i < len(video_ids):
            time.sleep(jittered_delay(args.sleep, args.sleep_jitter))
