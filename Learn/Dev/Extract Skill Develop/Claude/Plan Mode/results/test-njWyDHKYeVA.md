...
back to understanding this end-to-end agent
system management. So Ayo, why don't you start to
explain the cost and capacity
[00:00:32] for [INAUDIBLE]. AYO ADEDEJI: Oh, yes,
...
your resources, such as GPUs, when you're
...
one, as well as observability.
[00:01:02] So these are all
the pillars we're going to be covering as
part of this lab, and yeah. ANNIE WANG: Yeah,
so before we go through how to
...
to use open model IO?
[00:01:31] AYO ADEDEJI: Yeah,
you mentioned, you just talked about a lot of
...
solution for that. As you mentioned, you
can customize the model. So a lot of use cases have
very domain-specific data,
[00:02:02] where you can improve
...
and system instructions of that nature. So those are the benefits
...
if you're using a closed
[00:02:33] model like Gemini,
...
to this model strategy-- AYO ADEDEJI: Yeah, there's
...
so focused on model strategy
[00:03:02] is, at the end of
...
you use Gemini as the brain behind
...
lab, we're going
[00:03:30] to learn how can you use Gemma
4 as the brain behind an agent. ANNIE WANG: Yeah, so
...
to choose your model. And when you choose
the model, you
[00:04:00] can think about the performance
...
4 model, this service, vLLM and Ollama. But yeah, why we have
this two different way?
[00:04:31] What is the real difference
...
production use cases. It comes with Paged
...
[00:05:02] vLLM is great for production. And there's other elements
...
focusing on Ollama and vLLM. ANNIE WANG: And
...
started with Ollama.
[00:05:32] So as you can see from this
diagram, what we want to do is we want to first pull
the model from Ollama. And then we want to build
...
to your deployed Gemma 4.
[00:06:00] And if you're interested, let's
start by going through this lab. So we will have this link
...
[00:06:30] with L4 accelerators and such. Because of that, we won't be
...
authenticate it, and just making sure
to refresh the page to refresh the token so that
it will keep you authenticated. And to get start, let's
just set up our environment
[00:08:00] so that we are ready to deploy
...
sure this Gmail account is the same account you have
your billing account connected. And next, we need to
clone this to repo.
[00:08:31] We'll clone those to repo. And one is the
...
cloning two different repos. The first one
...
deploying our images to Artifact
[00:09:02] Registry and such. So essentially, it has a bunch
...
using for the boss fight later on in the lab. So once we build our
...
time we get to that point, our boss fight is ready to go.
[00:09:31] ANNIE WANG: Yeah, let's come
back to this setup page. And next, you need to initialize
a new project so that you have a Google Cloud project ready. And you can see that you always
...
this full setup complete.
[00:10:00] But for you, if you're
running the script, you'll probably notice that when
you fetch the billing account, it has a fetch failure. That because it's looking
...
I just click this Search icon
[00:10:30] in the search bar. I will search resources. So when I type resources,
I have something called Manage Resources. So when I open the
...
also want to make sure that again, the Gcloud is
set to the right project. So what we just
did was making sure that the Gcloud is set to
the right project, which
[00:11:31] should match your Agentverse
...
your-- later on, moving to vLLM, we want to store our model
in Google Cloud Storage. And we want to
deploy to Cloud Run.
[00:12:02] We want to enable Cloud
...
add is that just by enabling these
APIs does not mean you're going to be charged
immediately for these APIs. Essentially, you're
just enabling them to be used in the project.
[00:12:30] A lot of people ask, if
...
thing to keep in mind. ANNIE WANG: Yeah. And it's coming back to this. Yeah. So now, we successfully
...
this diagram that we
[00:13:02] want to build an image of-- we want to first pull the model
from Ollama and build the image and later, deploy this
to image repository. Yeah. I don't know why we want
to have this design. Is there any other
...
is the build process that
[00:13:31] we're going to be enabling
with Cloud Build that allows us to programmatically specify
each of these steps, and then we can go ahead and
...
[00:14:05] And next one is set
up the permission. I don't know, what
...
metaphor I can think of is, you can think of
...
same account for easy demo.
[00:15:30] But in production, you can
have different robot account and different service account
for different permission configuration. And the last step is to warm up. We need to run this warm
...
use that, stay tuned.
[00:16:01] We will talk about
it in VLM deployment. AYO ADEDEJI: Yes. ANNIE WANG: Now, it's finished. Let's start host
...
testing any model because Gemma 4 is baked in Ollama.
[00:16:31] And you can see, it's very easy. First, we want to create this
Dockerfile build the image. And with only this
...
coming back to my terminal, and paste it.
[00:17:00] So now, we just create
a Dockerfile folder. And what's next is
we want to create this cloudbuild.yaml file. So Ayo, maybe you can explain,
what is cloudbuild.yaml file, before we explain what are
...
the Dockerfile that
[00:17:31] defines our container image. Now, we're going to build
the container image. In step one. We're going to push the
...
this Dockerfile to explain how to pull this image.
[00:18:00] So you want to build
...
anything we need to be aware of when we're trying
to deploy our image to Cloud
[00:18:31] Run? AYO ADEDEJI: Yeah. Cloud Run is a really
...
going to be storing Ollama or Gemma 4 model on the image. You have to have a lot
of memory capacity. So we're working with a
2D version of Gemma 4. So 16 gigabytes is
sufficient memory for that.
[00:19:03] So that's why we're
...
we're just trying to optimize for how we're going
...
[00:19:31] But for cost purposes, just so
you're not allocating more GPUs than you need, we're kind
of just minimizing it to one machine per
service for this lab. ANNIE WANG: Yeah. Also, we have this allow
...
Console in the Cloud Build
[00:20:30] to see the process. So now, you can see that we just
finished the environment setup. And now, we're waiting
for the build to complete. It's trying to pull
...
break and grab some coffee.
[00:21:03] But yeah, we'll be back once
our Cloud Build job completes. ANNIE WANG: Yay! It finished! Finally. And as you can see, that we
...
this URL, right?
[00:22:00] We have this URL, and we want
...
is to try to grab that URL. You can also grab this
...
[00:22:31] you want this Gemma 4 model
answering this question. And let's see, what
is the response? I'm just going to copy this
and paste it to the terminal. Time to verify if
...
[00:23:01] So throughout this
lab, we're setting that set_env multiple
times at the very top. So that's the reason why it
takes a little bit longer for each step. But it's just important
...
[00:23:30] But maybe you're not exactly
the same response here, because AI cannot generate the
...
on Cloud Run with Ollama
[00:24:03] is working. All right. So Ayo, you know that
we are using a Ollama, and it's really
...
is because we're baking it into the container image, if
you want to change the model version or the
parameters of the model, so you want to go from 2 B to a
larger version of the same model
[00:25:01] to a different model, you have
...
running these models locally, you may want to use Ollama. But if you're baking it
...
changes to an extent,
[00:25:31] but that's just one
...
the performance, that is where you should really
...
is a little bit similar where we have this model.
[00:26:01] The Ci pipeline is
kind of similar. You want to build the image. But this image does
not contain the model. It only contain the vLLM code. And then once you
...
Cloud Storage FUSE to mount
[00:26:30] the bucket as a local folder. So anything you change in the
local folder will reflect it. Ayo, can you explain more
about Cloud Storage FUSE? AYO ADEDEJI: Yeah. So Cloud Storage
...
that you would read from a GCS bucket, any files that you
write to that local folder
[00:27:30] is almost as if you're writing
...
Cloud Storage FUSE. Let's get to step five. All right. Let's get started by
first, download this model from Hugging Face, and then
save that to Cloud Storage.
[00:28:01] So let's get started. And what you need
...
Account over here. If you already have it,
you can just click Login. And now, you're in
this page to log in. And what's next is you
need to click on this link to generate a token.
[00:28:32] So now, I'm at this page. I'm going to click,
...
you can just use the Read rule. So we can choose Read for this
...
command so that it will save
[00:29:01] the token to Secret Manager. And first, we just
...
to run the script. Essentially, behind
...
essentially, it's eponymous.
[00:29:30] It speaks for its name. It's our best practice way
for managing secret API keys in Google Cloud context. So if you're working in a
Google Cloud environment, rather than you storing your API
...
[00:30:02] So it's a way of preventing
you from having to actually make visible your secrets
...
your application secrets. ANNIE WANG: All right. So now, if you're
coming back, it's asked you to paste your
Hugging Face token.
[00:30:30] So what we want to
...
Cloud Console, over here. All right. It says, success.
[00:31:01] And we can double
...
so we can verify it by go to Google Cloud Storage.
[00:32:00] Store, Cloud Storage. You can Search Cloud storage
here, and making sure we select Agentverse. And here in the
...
Guardians storage ready,
[00:32:31] and then we can
...
here is essentially, we're not going to go
...
to download, actually,
[00:33:00] from Hugging Face. So we're specifying
the model that we want to download based off the ID. We're pip installing the
...
model ID, and then ultimately, just copying
that model to the GCS bucket.
[00:33:31] Yeah. ANNIE WANG: Great. So let's go ahead and create
...
this Cloud Build download file with our command. And next is, let's using Gcloud
Build command to run this Cloud
[00:34:06] Build download YAML file
...
for this build environments. Yeah, it finished! So what we've done so far
is, we have this token
[00:35:03] so that we can install this
...
everything in the bucket. I'm going to refresh the page. You can see. Yay! We have Gemma 4. Everything downloaded
in our Cloud storage. Great.
...
listing out the files. ANNIE WANG: Yeah.
[00:36:00] So now, we verified it. So the next thing is, we
...
having to pull the data over the public internet. So it's going to traverse
through our private subnet. So it's a really smart
way of keeping data secure when you have,
again, connections
[00:37:00] between different
...
the right version of the transformers
...
entry point for the VLM server
[00:38:31] when it's being deployed on
Cloud Run, and that's it. So pretty much, all
we're doing, again, is we're specifying the
VLM default container image that we want to use, one
...
Gemma 4 documentation vLLM, you'll notice that
it's using 5.50. So it's just a good way
of being consistent. But yeah.
[00:39:30] ANNIE WANG: Got it. Thank you. So let's configure it by
...
as always, we want to build the image with Docker. So we want to build this image. So we have this image with vLLM. And then next is, you want to
push this to Artifact Registry,
[00:40:00] like this. And then this next
...
setting here for vLLM, compared to Ollama settings previously? AYO ADEDEJI: Great question. It's pretty similar. There are some
slight differences, especially because we're taking
a mounted approach with vLLM. So especially before, we have
the model weights in GCS,
[00:40:32] and we're mounting it on
...
we're pretty much using the same exact model with the
...
[00:41:01] and then two, to actually
mount the Cloud Storage bucket as a local path. So you can see, there's
...
Storage FUSE explanation before.
[00:41:32] And you can check these
flags over here by configure the volume for enable
...
deploy to Cloud Run.
[00:42:03] Yeah. ANNIE WANG: Yeah. And it probably take 20
minutes or so, right? AYO ADEDEJI: Yeah. 20 to 30 minutes. ANNIE WANG: All right. Looks like it's finished. AYO ADEDEJI: Yay. ANNIE WANG: Yay. So what's next is, let's take a
...
[00:42:33] you pick this project. And you can see
that we have Ollama that deployed from the previous
...
the same question,
[00:43:00] like, as our Guardian
of Agentverse, what is my primary duty? And we want to verify if this
...
[00:43:31] we have some cold start. AYO ADEDEJI: Yeah. So let's look. So we see the response. As a Guardian of Agentverse,
...
looks like it's working. Yeah. ANNIE WANG: Great. All right. So just have a quick
overview of what
[00:44:02] we've done so far with vLLM. So first, we
...
advantages or disadvantages of this approach because
compared to Ollama,
[00:44:31] Ollama is very easy to
...
the trade-off with vLLM? AYO ADEDEJI: No. That's a great summary
where essentially,
[00:45:00] with VLM, with the process
...
same endpoint at a time, that warm instance
...
meant for the first call to that newly created machine.
[00:45:30] But, yeah. ANNIE WANG: So here
...
has multiple GPU support. However, if you use the
...
[00:46:03] we have-- and for today's
...
pull model from Ollama and host on Cloud
...
actually be extracting metrics such as, how many tokens
are being generated, what is the GPU
utilization, and so on. So we're going to be
learning a lot tomorrow. I hope you guys all stay tuned.
[00:47:30] And-- ANNIE WANG: That's a
really good summary. So by the end of
these two episodes, you're going to learn everything
about this end-to-end agentic system management. Yeah. AYO ADEDEJI: Yeah. So stay tuned. But until next time, I'm Ayo. ANNIE WANG: I'm Annie. Bye. [MUSIC PLAYING]
