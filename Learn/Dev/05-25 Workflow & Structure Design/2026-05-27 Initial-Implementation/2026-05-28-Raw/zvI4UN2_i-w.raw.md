...
id: zvI4UN2_i-w
type: youtube
url: "https://www.youtube.com/watch?v=zvI4UN2_i-w"
...
fetched_at: "2026-05-25T13:06:38+00:00"

# === visual ===
thumbnail: "https://i.ytimg.com/vi/zvI4UN2_i-w/maxresdefault.jpg"
thumbnail_image: "Learn/Dev/05-25 Workflow & Structure Design/2026-05-27 Initial-Implementation/2026-06-03-Thumbnail/zvI4UN2_i-w.jpg"
...
about the term agents a lot.

[00:00:28] Many people are super
enthusiastic about it. Bill Gates said
...
terms of number of stores. But it didn't take
long for people to realize that it was,
at least at the time, more of a prototype, and that
there was a lot of limitations

[00:01:42] from a production system. If we take one step back
...
classic schematic illustration from Russell and Norvig. And it has been part of
...
on the environment. Then it becomes an agent.

[00:02:48] That seems reasonable,
...
at its own reasoning process and then autonomously
...
[00:03:23] Fundamentally, I think there
are two main competing views in the community. The dominant view, I call
it an LLM-first view. So first, I have an LLM. It's amazing. It's very powerful. It knows a lot of things. So let's make that
...
[00:03:55] And I think that's also
where a lot of the sentiments that the agents are just thin
...
through the new lens of LLMs.

[00:04:59] And also, we need to
tackle the new challenges and opportunities. Now, we can do
...
to be more chatty and release it to the public.

[00:06:02] So I think that really shows
...
using language for reasoning is probably a bit more
unique this time around.

[00:06:36] It used to be the
...
no, but with a lot a bit more thinking, then it realizes
that the answer is actually yes.

[00:07:04] But the way I see this
...
where you pay a fixed amount of compute for any decision. So you only have one shot,
and you have to commit to it. And especially in the
context of agents,

[00:07:37] reasoning is not just for
...
this new generation of language agents with the previous agents.

[00:08:08] All we need to do is to add
...
can also reconcile things like self-reflection. Now, it becomes just a
meta reasoning action, it's a reasoning over
the reasoning process,

[00:08:41] just like the
...
of the use of reasoning here, because this is
such an overloaded term. If we don't properly define it,
then people will frown upon it.

[00:09:11] So I'm sympathetic
...
the famous late Daniel Kahneman in thinking
...
[00:10:19] when it looks at this
...
what you can immediately infer from what you see, and
...
to call this reasoning.

[00:10:51] And then one may alternatively
call these thoughts to avoid this overloaded
term, but then the risk is with the further
anthropomorphize these machines. So I prefer reasoning
...
autonomous agents, LLMs agents, and so on and so forth.

[00:11:21] But I truly think language
agents is probably the most discrete characteristic
name for this kind of generation of agents, because
...
lifting, like the reasoning and communication part. And what about LLMs agents?

[00:11:51] That's probably the most
...
beyond the LLM, but this need, this capability will remain.

[00:12:20] So LLMs could be
a means to an end. In that sense, it's
less as fundamental. So language agents is still
a more appropriate name. If we take one step back and
...
organs and represent them

[00:12:55] in a unified neural
representation to reconstruct the
world around us and also to support symbolic
...
[00:13:27] like perception in
single modalities. Only recently was this
...
by the logical language we use.

[00:14:26] Here, the agent or the
model can essentially encode almost
...
[00:16:27] like safety, evaluation,
synthetic data is both a challenge and an
...
claims and main contributions into this framework.

[00:16:52] So that's the introduction. And for the rest of
the talk, we will try to cover three main
aspects of language agents. How we model long-term
...
using a word model. OK. So let's start with memory.

[00:17:26] We'll talk about HippoRAG. The main content will
be from HippoRAG, but we'll talk about a lot
...
Bernal, and in collaboration with Michi from Stanford.

[00:17:48] So if we look at how
humans or really, most other animals that have a
nervous system, how they learn, it's really fascinating that
we are all this 24/7, nonstop, lifelong learners. And all we learn is
...
[00:18:27] Without it, we are
nothing, which, I think, is very profound, because
...
in forming long-term memory. So we're really 24/7 learners. Even when we are
...
of learning capacity in machines

[00:19:47] or especially, in agents as
...
self-improve, in some sense. However, that's very hard
...
[00:20:27] in these models, when you
are learning new things, it often has unintended
...
knowledge model editing. And a hot topic in
LLMs model editing

[00:21:05] is the so-called ripple effect. So if you want to
...
[00:22:22] networks, that makes
continual learning very hard. The human brain
and animals somehow figure out a way to do
...
using some mechanism as we go.

[00:23:23] Of course, this is called a RAG,
Retrieval Augmented Generation. But for that to work there is a
prerequisite condition, that is, when you retrieve some external
...
really make long-term memory work for LLMs. Most people know that RAG is
the de facto solution today.

[00:25:37] You are given something that is,
...
them for the current context, and so on and so forth. So to illustrate some
...
works on the neuroscience

[00:26:46] of Alzheimer's. So you have two
...
parts of the information. So you don't happen to have
...
person works at Stanford.

[00:27:19] This passage says that this
person works on Alzheimer's. And then you will see,
there is one person who works at Stanford and
on Alzheimer's, but it's in separate documents. You don't happen to have one
...
similarities one by one.

[00:27:52] Then you will find,
all of these passages are equally likely
because each of them captures precisely 50%
of the information. And as a result, the
...
[00:28:27] But our human memory
doesn't work that way. Somehow, if we have come
...
[00:28:56] all of this other information
to arrive at that conclusion. So in that sense,
embedding based in RAG works very differently
from human memory.

[00:29:11] Now, to study how exactly how
...
reliving the moments, because it's just really
...
[00:30:22] that associates all these
disparate memory units together. And that's what we are
trying to mimic here. This kind of separation
...
episodic memory, this second

[00:30:58] and the second before, they
are very similar to each other. How can you differentiate
...
recall this whole fact

[00:31:27] that this person is
associated with both of them. And that's due to
...
[00:31:56] I won't get into
too much details, so just at a high level, in the
hippocampal indexing theory, there are three main parts-- there is the neocortex,
...
say, you have these passages as inputs. We use an LLM to serve
as a neocortex that

[00:33:04] will do open information
...
or a predefined schema, or anything like that. Everything is extracted by an
LLM from the raw experiences.

[00:33:36] So we built a knowledge
...
retrieval to find the similar nodes in my index. And then those nodes become the
...
algorithm we're using here

[00:34:55] is Personalized PageRank. OK. So for people who
don't quite remember what a Personalized PageRank
...
closer to these seed

[00:35:27] nodes or especially, those
that are in intersection of multiple seed
...
equally important for it to actually work, to work very
well and better than existing

[00:36:36] solutions in practice. And that's what we
...
big boost as well. OK. And to better understand where
...
[00:37:50] So if you think about,
...
this question looks like? You will see, it's not just a
...
[00:38:27] to Alzheimer's, that's also
a many to one relationship. So there are many people
...
[00:39:11] OK. So that was HippoRAG. That was published in NeurIPS,
which is just two months ago, but in today's pace,
it feels like ages ago. So I'm very excited to
share that HippoRAG v2 is coming very soon. And so a problem with HippoRAG,
...
[00:39:54] like ColBERT v2 or Contriever. But recently, there have been
many large embedding models, like Grit, like NV-Embed. And if you actually compare
...
[00:40:28] like just some very simple QA
or some discourse understanding, and so on and so forth,
...
parametric continual learning for LLMs is very hard. But fortunately,
non-parametric memory like RAG could be a promising solution. And the recent trend in RAG
is to add more structures

[00:41:40] to embeddings, like
...
between disparate pieces of information. OK? So that's memory. And I think we're
still quite far away from developing a very
sophisticated memory system,

[00:42:13] but we are getting there. But there are still
...
fundamental aspects in my opinion. Then let's get to another
...
[00:43:25] let's say, the language model
has memorized or know these two atomic facts that
...
Barack's wife is Michelle. Michelle was born in this.

[00:43:59] So therefore, the
answer is this. But we want to push the
...
mechanism of these LLMs is mostly just token generation,
but implicit reasoning

[00:44:35] is still part of the
...
least, not for now. So in training time, model
has to compress the data. It has to do implicit reasoning
in order to reasonably predict

[00:45:13] the next token. So no matter what, I think
...
of the facts and the rules from
...
hypothesis, as it is in my mind,

[00:45:49] is like this. You start with a
capable base model. If the base model is not capable
enough, then RL won't do much. So you start with the capable
base model, like DeepSeek v3. And it probably
...
circuits in the parameters.

[00:46:22] Then reinforcement learning
was these verifiable rewards, is just to incentivize
the model to learn to use the right
...
[00:46:57] So back to implicit
reasoning, before our work, there were a bunch of
...
[00:47:26] Like, Trump is 78,
Biden is 82, then Trump is younger or
...
and never truly reason or plan, narrative. But we had some
...
[00:48:15] and to see whether we can
...
acquisition of this skill? And then what factors? Is that the scale of the
training data, the distribution of the training data, or
the model architecture that control the acquisition
of implicit reasoning?

[00:48:44] OK? So with these two questions,
...
a standard decoder only transformer in
...
[00:49:18] Then for the data we'll be
...
the problem we want to study. Let's just consider composition. We study both composition
...
[00:49:49] We set R to 200. So then it's a bunch
of nodes and then these different
relations like this. Then these are the atomic facts. Like, superstition
...
have this composition effect,

[00:50:26] like head entity,
r1, r2, tail entity. So it's like Barack,
wife, born in, 1964. So this composition, and you can
...
just decompose this for you. First of all, we
want the model, which is the decoder-only transformer,
to learn inductively.

[00:51:06] So just give a bunch
...
generalization settings, what we call the
in-distribution setting and then the out-of-distribution
setting, or this setting

[00:51:40] can also be called
...
facts in the ID test

[00:52:55] set, like, Barack,
wife, born in, 1964, maybe you have not
...
compositional fashion, being composed with
other relations. So in that sense, it's a
stronger generalization setting,

[00:54:08] so that's why it's called the
...
goal for this learning of deduction rules. Now, with all this vanity
set up in minds, let's look at some fun parts, look at
some interesting takeaways from our investigation.

[00:54:41] The first surprising
...
this figure for now. This curve is the
...
[00:55:24] And if you keep training the
model way beyond the saturation or overfitting, the overfitting
...
100% test accuracy on composition in the ID split. Similarly, for comparison,
the overfitting

[00:56:06] happens very early
...
[00:56:27] So that's quite interesting. This transformers,
...
of generalization varies by the reasoning type.

[00:57:04] For compositional
...
were already some studies that looked at grokking. And then try to understand,
why grokking happens? And under what conditions
would grokking happen?

[00:57:40] Then one common belief
...
to study this hypothesis very carefully. Then we find that actually, it's
not the data size but the data distribution that matters. So in these experiments,
on the right, we keep the--

[00:58:17] so there is two variables. There is the total
...
is the ratio of the inferred versus atomic facts. So if this number
...
entities or the total data size.

[00:58:52] Then you will see, the
speed of generalization is roughly the same when you
increase the total data size. It's, more or less, the same. However, when you keep
...
correlates with this ratio. If you have a higher ratio,
then the generalization

[00:59:33] will happen faster. And at some point,
...
we have not answered. So why does grokking happen? What exactly happens during
this long grokking process?

[01:00:10] What's going on
...
interpretation part of this study. So we will use some
...
[01:00:50] and multiply it with the
output embedding matrix so that we can get a
distribution over the output vocabulary. So we get a peek into what
this internal representation is about. Then we will also use the
so-called causal tracing.

[01:01:14] In a nutshell, this
technique allows you to quantitatively
measure the amount of the impact of a certain
internal state to the output. So it ranges from 0 to 1. If it's closer to 1, that
...
[01:01:56] now, we can try to
find or discover, rather, that the corresponding
...
facts in the first few layers to the extent that it can
reliably predict b at layer 5. Then another thing that
needs to be done here

[01:03:12] is that it needs to defer
...
found b, the bridge entity. OK? So once the bridge
...
bridge entity and the data

[01:03:50] processing of r2 to predict t. So that's the generalizing
circuit for composition, it has two clear stages. And that will also determine
...
entity, like, this is Trump,

[01:04:27] this is Biden. And then Trump is v1 is 78. Biden is 82. Then the prediction is like,
...
retrieve the attribute values, like 78 and 82. Then the upper layers will
use these retrieved values to do the comparison and then
to predict the final answer,

[01:05:11] whether it's smaller
...
atomic facts has been seen during training to
be composed with other facts.

[01:07:07] In that situation, the
...
in the upper layers, because that requires
extra effort. That's why ODD generalization
of composition never happens. Well, for comparison, you don't
have this stage of processing.

[01:07:43] So you only need to store one
...
comparison does happen. OK? And to further validate
...
fact in the upper layers,

[01:08:16] then we just need to do some
cross-layer parameter sharing. So you tie the weights
of the lower layers with the weights
of the upper layer, so it's a parameter sharing. And then if the lower layer
...
[01:09:51] Let me first say this. So we believe, grokking is the
phase transition from rooted learning to generalization. So grokking starts
...
[01:10:28] so the training
loss will be zero, but it doesn't mean
...
look at is this figure. We look at two things. So S, 5, r1 is this position.

[01:11:12] This is S stands for states. So this is a state of layer
5 as the position of r1. So it's this one, S5, r1. Then we can use
logical dense to decode what's captured in that state
representation, in that state. You can see, the
...
[01:11:55] But basically, it
just tells you, the ranking of the bridge
...
does not have r2 there. So it has to be here, but
...
process, you can see, the r2 gradually increases.

[01:13:54] So at the end of a grokking,
it always predicts r2. Then you have t here,
you have r2 here, then you can actually
...
strength of each state at the beginning of grokking.

[01:14:34] And this is after grokking. Now, we can calculate the
...
the bridge entity be here, it does not use it. It just did rote learning, so it
directly predicts the t entity. But the grokking
process is the process of the model learning to use
this bridge entity properly,

[01:15:12] so giving it a stronger
...
validates the hypothesis that this grokking
...
[01:15:50] do any proof of any
sort, but you can roughly understand why this kind
of grasping behavior happens through the
circuit efficiency and the regularization. The generalizing circuits
...
even after overfitting,

[01:16:21] the regularization
term will decrease. And then it will gradually
favor the circuit with higher efficiency, which
is the generalizing circuit. So that's why you find it still
...
purpose, I think it's enough to have this definition. Then with this definition, we
...
[01:17:54] So now, we can express our goal
in natural language as opposed to some formal language,
which is usually much more limited in expressiveness. And also, we have a
...
[01:18:21] And we will see some
examples soon later. Then because of those,
it becomes increasingly hard to automate the goal test. Imagine, you have a
...
dynamically populated on each web page.

[01:19:41] So if you go to a
...
characteristics, but we'll skip here. So I think these are some of
the general trends for language agent planning. And I think, these
are good trends.

[01:20:11] Yeah, it makes things
...
to talk about this, maybe, very quickly. But for people who are
interested in web agents or computer user
agents, I encourage you to look at these
series of work. Like Mind2Web, the first
LLM-based web agents, and then

[01:20:42] SeeAct, where we first
...
pixel-level operations on the screen. And this minimalist
design actually works the best across the board. And then we will talk
about WebDreamer,

[01:21:11] which is model-based
...
here is a web page. Then at each stage, you have
...
about it, then make a decision,

[01:21:44] and commit to the
...
stuck in some bad state. And there's no way out. Then naturally, when
we talk about planning, we just think about
search or tree search. So for tree search, compared
with reactive planning,

[01:22:11] you can do backtracking. You maintain a value assignment
...
explore another branch. So that gives you more
...
[01:22:47] that makes backtracking
...
this state-changing and irreversible actions are
...
state-changing actions. You can place an order. You can make a return.

[01:23:17] You can create an account that
...
universal magical Undo button that you can just try a bunch
...
[01:23:47] So ideally, we want to
do model-based planning. Imagine, you have a world
model that at each stage, you can trigger the world
model to simulate the outcome of each candidate action. Then that gives you a chance
...
it, assuming it's safe.

[01:24:28] And then you get
to another stage, then you can do
...
[01:24:56] dynamics. So basically, if I do this right
now, what would happen next? Very simple. Then if it's that
...
[01:25:57] if we consider that there are
billions of other websites out there. In that sense, we need
...
that, hey, this is a shirt. And this is probably a product.

[01:26:28] So the next state will probably
be about some product details. And because it's
...
future directions I find it interesting. I think, memory,
personalization,

[01:29:32] and continued
...
these external actions and environmental states? And that's a big, open question.

[01:30:02] And I expect, there will be a
...
deep research agent can go there, visit there,
and get tricked by it,

[01:31:04] and then maybe reveal your
...
exogenous risks. So these are the risks from
the external environment.

[01:31:36] OK? But there are also a lot
...
big is being baked here. I think this will become
a huge thing in 2025. Then the, also,
workflow automation. And personally, I'm very
excited about developing agents

[01:32:11] for sciences. These are all very exciting. So for a more comprehensive
...
like slides and videos, are available on our website. I thank all of my sponsors, and
