...
> [!note]
> Several findings that *started* in this test — language defaulting to English, the `raw_file` link bug, the TL;DR being too dense as a paragraph — are already written up in [[Learn/Dev/05-19 Summarize Skill Develop/test reviews/test-1/revise|test-1/revise.md]] because all the revisions landed in one continuous editing pass. This file focuses on what's *not* there.
